...
$read_query="SELECT * FROM `blogs` WHERE `blog_file`='$blog' AND `blog_category`='education'";
...
<div class="w3-sidebar w3-bar-block w3-card w3-animate-left w3-text-white" style="display:none;background-color:rgb(51,63,80);" id="mySidebar">
  ...
  <?php 
  $content_query="SELECT * FROM `blogs` WHERE `blog_category`='education'";
...
$user_run=mysqli_query($blogs_con,$user_query);
...
<div class="w3-dropdown-click">
    ...
    if(isset($_SESSION['ssid']))
    {
      $follower=$_SESSION['ssid'];
      $following=$blogs1_data['author_id'];
      $check_query="SELECT * FROM `follower_following` WHERE `follower_id`='$follower' AND `following_id`='$following'";
      $check_run=mysqli_query($blogs_con,$check_query);
      if(mysqli_num_rows($check_run)>0)
      {
        ?>
        <input type="text" style="display:none" value="<?php echo $following;?>" id="unfollowing_id">
  <input type="text" style="display:none" value="<?php echo $follower;?>" id="unfollower_id">
        <button type="submit" class="w3-border w3-blue w3-hover-white  w3-text-white w3-button w3-round-large" id="unfollowed">Following</button>
      <?php
      }
else{
  ?>
  <input type="text" style="display:none" value="<?php echo $following;?>" id="following_id">
  <input type="text" style="display:none" value="<?php echo $follower;?>" id="follower_id">
  <button type="submit" class="w3-border w3-white w3-hover-blue  w3-text-gray w3-button w3-round-large" id="followed">Follow</button>
  <?php
}
...
$("#followed").on("click",function(f){
  f.preventDefault();
  var fr_id=$("#follower_id").val();
var fg_id=$("#following_id").val();

  $.ajax({
url:"follower_following.php",
type:"post",
data:{follower_id:fr_id, following_id:fg_id},
success:function(data){
    if(data==1)
    {
      $("#followed").html('Following');
    }
    else
    {
      alert("Not followed!");
...
$("#unfollowed").on("click",function(f){
  f.preventDefault();
  var unfr_id=$("#unfollower_id").val();
var unfg_id=$("#unfollowing_id").val();

  $.ajax({
url:"unfollow.php",
type:"post",
data:{unfollower_id:unfr_id, unfollowing_id:unfg_id},
success:function(data){
    if(data==1)
    {
      $("#unfollowed").html('Follow');
    }
    else
    {
      alert("Not followed!");
